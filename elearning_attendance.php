<?php
/**
 * صفحة تسجيل الزيارات الصفية للمعلمين
 */
...
$page_title = 'تسجيل زيارة صفية جديدة';
$success_message = '';
$error_message = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $attendance_students = $_POST['attendance_students'] ?? '';
    ...
    if (empty($num_students) || $num_students < 1) $errors[] = 'يرجى تحديد عدد الطلاب';
    if (empty($attendance_students) || $attendance_students < 0) $errors[] = 'يرجى تحديد عدد الطلاب الحاضرين';
    if (!empty($num_students) && !empty($attendance_students) && $attendance_students > $num_students) {
        $errors[] = 'عدد الطلاب الحاضرين لا يمكن أن يكون أكبر من العدد الكلي';
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $result = query($sql, [
                $academic_year_id, $school_id, $subject_id, $teacher_id, $grade_id, $section_id,
                $lesson_date, $lesson_number, $num_students, $attendance_students, $attendance_type, 
                json_encode($elearning_tools), $lesson_topic, $attendance_rating, $_SESSION['user_id'], $notes
            ]);
            
            global $pdo;
            if ($pdo->lastInsertId()) {
                $success_message = 'تم تسجيل الزيارة الصفية بنجاح';
...
<div class="min-h-screen bg-gray-50 py-6">
    <div class="max-w-4xl mx-auto px-4">
        <!-- عنوان الصفحة -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">تسجيل زيارة صفية جديدة</h1>
            <p class="text-gray-600 mt-1">تسجيل ومتابعة الزيارات الصفية للمعلمين</p>
        </div>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">عدد الطلاب الكلي *</label>
                        <input type="number" name="num_students" id="num_students" min="1" max="50" 
                               value="<?= htmlspecialchars($_POST['num_students'] ?? '') ?>" 
                               required class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                               placeholder="أدخل العدد الكلي للطلاب">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">عدد الطلاب الحاضرين *</label>
                        <input type="number" name="attendance_students" id="attendance_students" min="0" max="50" 
                               value="<?= htmlspecialchars($_POST['attendance_students'] ?? '') ?>" 
                               required class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                               placeholder="أدخل عدد الطلاب الحاضرين">
                    </div>
                </div>
                ...
                <div class="mt-4 p-3 bg-gray-50 rounded-md">
                    <p class="text-sm text-gray-600">
                        ...
                        سيتم حساب نسبة الحضور تلقائياً بناءً على الأرقام المدخلة
                    </p>
                </div>
            </div>
...
function updateGrades() {
    ...
    if (schoolId) {
        fetch(`api/get_grades_by_school.php?school_id=${schoolId}`)
            .then(response => response.json())
            .then(data => {
                console.log('بيانات الصفوف:', data); // للتشخيص
                if (data.success && data.grades) {
                    ...
                    console.log('لا توجد بيانات صفوف أو خطأ:', data);
                }
            })
...
document.getElementById('grade_id').addEventListener('change', function() {
    ...
    if (gradeId && schoolId) {
        fetch(`api/get_sections_by_school_grade.php?school_id=${schoolId}&grade_id=${gradeId}`)
            .then(response => response.json())
            .then(data => {
                console.log('بيانات الشعب:', data); // للتشخيص
                if (data.success && data.sections) {
                    ...
                    console.log('لا توجد بيانات شعب أو خطأ:', data);
                }
            })
...
// حساب نسبة الحضور التلقائي
function calculateAttendancePercentage() {
    const totalStudents = document.getElementById('num_students').value;
    const attendingStudents = document.getElementById('attendance_students').value;
    
    if (totalStudents && attendingStudents) {
        const total = parseInt(totalStudents);
        const attending = parseInt(attendingStudents);
        
        if (attending > total) {
            alert('عدد الطلاب الحاضرين لا يمكن أن يكون أكبر من العدد الكلي');
            document.getElementById('attendance_students').value = total;
            return;
        }
        
        const percentage = total > 0 ? ((attending / total) * 100).toFixed(1) : 0;
        
        // إظهار النسبة للمستخدم
        let percentageDisplay = document.getElementById('attendance_percentage_display');
        if (!percentageDisplay) {
            percentageDisplay = document.createElement('div');
            percentageDisplay.id = 'attendance_percentage_display';
            percentageDisplay.className = 'mt-2 text-sm text-gray-600';
            document.getElementById('attendance_students').parentNode.appendChild(percentageDisplay);
        }
        
        percentageDisplay.innerHTML = `<i class="fas fa-chart-pie ml-1"></i>نسبة الحضور: ${percentage}%`;
        
        // تلوين النسبة حسب المستوى
        if (percentage >= 90) {
            percentageDisplay.className = 'mt-2 text-sm text-green-600';
        } else if (percentage >= 75) {
            percentageDisplay.className = 'mt-2 text-sm text-yellow-600';
        } else {
            percentageDisplay.className = 'mt-2 text-sm text-red-600';
        }
    }
}

// ربط الوظيفة بحقول الإدخال
document.getElementById('num_students').addEventListener('input', calculateAttendancePercentage);
document.getElementById('attendance_students').addEventListener('input', calculateAttendancePercentage);
</script>
